...
import {IOxionStorage, IOxionToken} from "./interfaces/IOxionStorage.sol";
...
import {OxionToken} from "./OxionToken.sol";

contract OxionStorage is IOxionStorage, OxionToken, Ownable {
    using SafeCast for *;
    ...
    function take(Currency currency, address to, uint256 amount) external override isLocked {
        ...
        reservesOfStorage[currency] -= amount;
    ...
    function settle(Currency currency) external payable override isLocked returns (uint256 paid) {
        uint256 reservesBefore = reservesOfStorage[currency];
        reservesOfStorage[currency] = currency.balanceOfSelf();
        paid = reservesOfStorage[currency] - reservesBefore;
