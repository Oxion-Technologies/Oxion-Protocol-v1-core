...
import {IOxionToken} from "./IOxionToken.sol";

interface IOxionStorage is IOxionToken {
    /// @notice Event emitted when poolManager is registered
    /// @dev There can be several PoolManagers, each with its own pool type
    ...
    /// @notice Called to check if the PoolManager is registered
    function isPoolManagerRegistered(address poolManager) external returns (bool);

    /// @notice Returns the reserves for a given ERC20 currency
    function reservesOfStorage(Currency currency) external view returns (uint256);
    ...
    /// @notice Called by the user to store surplus tokens in the Oxion Storage
